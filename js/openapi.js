    ...
    if (!VK.Api) {
        VK.Api = {
            ...
            checkMethod: function(method, params, cb, queryTry) {
                ...
                if (m === 'wall.post') {
                    var validAttacheRegexp = /(^https?:\/\/)|(^(poll|album|photo|video|doc|audio|page|note)-?\d+_-?\d+)$/, validAttachments = [], methodAccess, queryParams, query, timer;
                ...
                responseCb = function(response) {
                    if (response.error && (response.error.error_code == 3 || response.error.error_code == 4 || response.error.error_code == 5)) {
                        ...
                        var repeatCall = function(resp) {
                            ...
                            if (resp.session)
                                VK.Api.call(method, params, cb, queryTry + 1);
                        }
...
if (!VK.Observer) {
    VK.Observer = {
        ...
        publish: function(eventName) {
            var args = Array.prototype.slice.call(arguments), eventName = args.shift(), subscribers = this._subscribers()[eventName], i, j;
        ...
        unsubscribe: function(eventName, handler) {
            var subscribers = this._subscribers()[eventName], i, j;
...
if (!VK.Widgets) {
    ...
    VK.Widgets.Comments = function(objId, options, page) {
        ...
        var obj = document.getElementById(objId), params = {
    ...
    VK.Widgets.Like = (function(Like) {
        ...
        Like = function(objId, options, page) {
            ...
            var verticalBtnHeightWidth = {
                ...
                h: btnHeight
            }, ttHere = options.ttHere || false, isOver = false, hideTimeout = null, obj, buttonIfr, buttonRpc, tooltipIfr, tooltipRpc, checkTO;
            if (type == 'vertical' || type == 'button' || type == 'mini')
                delete options.width;
            if (autoWidth)
                params.auto_width = 1;
            function showTooltip(force) {
                if ((!isOver && !force) || !tooltipRpc)
                    return;
                if (!tooltipIfr || !tooltipRpc || tooltipIfr.style.display != 'none' && tooltipIfr.getAttribute('vkhidden') != 'yes')
                    return;
    ...
    VK.Widgets.Community = VK.Widgets.Group = function(objId, options, gid) {
        ...
        }, startHeight = options.mode == 3 ? 185 : (options.mode == 1 ? 141 : options.height | 0 || 290), rpc;
        if (options.wall)
        ...
        return VK.Widgets._constructor('widget_auth.php', objId, options, {}, {
            makeAuth: function(data) {
                ...
                if (options.onAuth) {
                    ...
                    for (var i in data) {
                        if (i != 'session')
                            vars.push(i + '=' + decodeURIComponent(data[i]).replace(/&/g, '%26').replace(/\#/g, '%23').replace(/\?/, '%3F'));
                    }
    ...
    VK.Widgets.Ads = function(objId, options, paramsExtra) {
        ...
        function adsOnInit(errorCode, adsParamsExport) {
            ...
            if (options.onAdsReady)
                options.onAdsReady.apply(options.onAdsReady, Array.prototype.slice.call(arguments));
        ...
        function adsAttachScript(adsScriptData) {
            if (!('vk__adsLight'in window)) {
                ...
                // false - do not publish initial onInit
            }
        }
    ...
    */
    ...
    VK.Widgets._constructor = function(widgetUrl, objId, options, params, funcs, defaults, onDone, widgetId, iter) {
        ...
        var ifr, url, urlQueryString, encodedParam, rpc, iframe, i, width = options.width === 'auto' ? (obj.clientWidth || obj.offsetWidth || defaults.minWidth) | 0 : parseInt(options.width || 0, 10);
        ...
        funcs.resizeWidget = function(newWidth, newHeight) {
            ...
            if (options.onResizeWidget)
                options.onResizeWidget();
        }
        ;
        ...
        Box: function(src, sizes, fnc, options) {
            ...
            var rpc = new fastXDM.Server(VK.extend(fnc, {
                onInit: function() {
                    ...
                    if (is_vk)
                        document.body.className += ' layers_shown';
                    VK.Util.Loader();
                },
                hide: function() {
                ...
                tempHide: function() {
                    ...
                    if (is_vk)
                        document.body.className = document.body.className.replace(/\b\s*?layers_shown\s*\b/, ' ');
...
if (!VK.Retargeting) {
    VK.Retargeting = {
        ...
        Event: function(event) {
            ...
            (window.Image ? (new Image()) : document.createElement('img')).src = 'https://vk.com/rtrg?p=' + this.pixelCode + (event ? ('&event=' + encodeURIComponent(event)) : '') + (metatagUrl ? ('&metatag_url=' + encodeURIComponent(metatagUrl)) : '') + (metatagTitle ? ('&metatag_title=' + encodeURIComponent(metatagTitle)) : '');
        },
        Hit: function() {
            this.Event();
        },
